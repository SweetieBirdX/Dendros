...
export function validateEdgeTargets(graph: DendrosGraph): ValidationError[] {
    ...
    for (const edge of graph.edges) {
        // Skip auto-generated edges without targets (they're waiting to be connected)
        const isAutoGenerated = (edge as any).autoGenerated || (edge as any).data?.autoGenerated;
        if (isAutoGenerated && !edge.target) {
            continue;
        }
